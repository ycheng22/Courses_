...
public class EarthQuakeClient2 {
    ...
    public void quakesWithFilter() { 
        ...
        Filter mf = new MagnitudeFilter(3.5, 4.5); // practice quiz: 2
        ArrayList<QuakeEntry> mList = filter(list, mf);
        Filter df = new DepthFilter(-55000.0, -20000.0); // practice quiz: 2
        ArrayList<QuakeEntry> dmList = filter(mList, df);
        //Filter df = new DistanceFilter(new Location(39.7392, -104.9903), 1000000); // practice quiz: 1
        //ArrayList<QuakeEntry> dList = filter(list, df);
        //Filter pf = new PhraseFilter("end", "a"); // practice quiz: 1
